...
their saved longitudes. Their radial endpoints and selected guides are bounded to
the participant track, outside the central aspect field and inside the zodiac ring.
Each track uses bounded deterministic label lanes and offsets, then selectable
dense groups with a count badge. All group members keep separate anchors.
...
placed after Relationship AI, provides a touch/keyboard alternative for
intersecting lines while continuing to control the chart selection.
